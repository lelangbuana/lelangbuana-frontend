...
class Home extends Component {
    ...
    render() {
        ...
        let listAuction = auctions.map((item, index) => {
            return (<Link to={`/auctions/${item.user}`} params={{id:item.user}}><CardAuction key={item.title+index} user={item.user} title={item.title} src={item.src} description={item.description}/></Link>  
        )
        })
